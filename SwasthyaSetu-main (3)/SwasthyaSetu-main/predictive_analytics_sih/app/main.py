...
load_dotenv()
API_URL = "http://127.0.0.1:8000/map"
# Logging
import logging
...
async def lifespan(app: FastAPI):
    ...
    # Load hybrid knowledge base
    app.state.knowledge_base = await load_hybrid_knowledge_base()
...
async def load_hybrid_knowledge_base():
    """Load hybrid treatment and nutrition knowledge base"""
    return {
...
async def root():
    return {
        ...
        "ai_status": "Available" if getattr(app.state, 'ai_available', False) else f"Knowledge-base mode"
    }
...
async def hybrid_assessment(data: Dict[str, Any]):
    try:
        ...
        if getattr(app.state, 'ai_available', False):
